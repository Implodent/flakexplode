{ lib, inputs, ... }: {
  perSystem = { config, pkgs, system, ... }:
    ...
    with lib;
    {
      ...
      config.fenix.toolchain = mkIf config.fenix.enable (
        let
          apply = chan: profile: components: target:
            ...
            in
            if target != null then applied.targets.${target} else applied;
        in
        if cfg.precise != null then
          fenix.toolchainOf cfg.precise
        else if cfg.manifest != null then
          (if builtins.isAttrs cfg.manifest then
